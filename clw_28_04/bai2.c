...
int main(int argc, char *argv[]) {
    ...
    // khoi tao mang pollfd de theo doi 2 input: ban phim va socket
    struct pollfd fds[2];

    fds[0].fd = STDIN_FILENO; // theo doi ban phim
    fds[0].events = POLLIN;
    fds[1].fd = sock; // theo doi socket
    fds[1].events = POLLIN;
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (bytes_read > 0) {
                ...
                // gui data
                sendto(sock, buf_keyboard, bytes_read, 0,
        ...
        if (fds[1].revents & POLLIN) {
            int bytes_received =
                recvfrom(sock, buf_receive, sizeof(buf_receive) - 1, 0,
            ...
            if (bytes_received > 0) {
                ...
                printf("Tu cong %d: %s", sender_port, buf_receive);
            }
        }
    }
