        ...
        if (fork() == 0) {
            ...
            while (1) {
                ...
                if (!client_check) {
                    ...
                    snprintf(out_filename, sizeof(out_filename), "out_%d.txt",
                             getpid());

                    // tao cau lenh: "lenh > out_{pid}.txt"
                    char sys_cmd[1050];
                    snprintf(sys_cmd, sizeof(sys_cmd), "%s > %s", buffer,
                             out_filename);

                    // goi ham system thuc thi
                    // vi du: ls > out_{pid}.txt. he dieu hanh se thuc hien lenh
                    // va ghi ket qua vao out_{pid}.txt
                    system(sys_cmd);

                    // doc file out_{pid}.txt de tra ve ket qua cho client
